...
void edf_scheduling(SystemInfo *sys_info, TaskInfo *task_info)
{
    ...
    */
    ...
    for (int i = 0; i < WCET_LEN; i++)
    {
        ...
        is_edf_valid_for_idx[i] = is_edf_valid(sys_info, task_info, i, 1);
    ...
    */
    ...
    for (int wcet_index = 0; wcet_index < WCET_LEN; wcet_index++)
    {
        ...
        float idle_time_percentage = ((float)total_idle_time / (float)sys_info->execution_time) * 100.00;
        printf("Total Energy Consumption: %.2fJ,\t %% of Time Idle: %.2f%%,\tTotal Execution Time: %ds\n", total_energy, idle_time_percentage, sys_info->execution_time);
    }
}
...
void ee_edf_scheduling(SystemInfo *sys_info, TaskInfo *task_info)
{
    ...
    */
    ...
    {
        // store which index are valid
        is_edf_valid_for_idx[i] = is_edf_valid(sys_info, task_info, i, ALPHA);
        if (is_edf_valid_for_idx[i])
        {
            valid_task_count++;
        }
    ...
    */
    ...
    // Loop through each valid WCET index and scale by 1/ALPHA
    for (int i = 0; i < valid_task_count; i++)
    {
        for (int j = 0; j < sys_info->num_tasks; j++)
        {
            // truncate the value to the nearest integer for ease of calculation
            valid_tasks[j].wcet[i] = (int)((float)valid_tasks[j].wcet[i] / ALPHA);
        }
    }
    // start scheduling
    // Loop through each valid WCET index
